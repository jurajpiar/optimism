...
contract OPContractsManagerDeployer is OPContractsManagerBase {
    ...
    // These functions split the monolithic deploy() into separate transactions
    // to fit within RSK's 6.8M block gas limit.

    /// @notice Deploys the AddressManager singleton.
    ...
    /// @notice Deploys the ProxyAdmin singleton.
    ...
    /// @notice Deploys the L1ERC721Bridge proxy.
    function deployL1ERC721Bridge(
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        _output.l1ERC721BridgeProxy =
            IL1ERC721Bridge(deployProxy(_input.l2ChainId, _output.opChainProxyAdmin, _input.saltMixer, "L1ERC721Bridge"));
        return _output;
    }

    /// @notice Deploys the OptimismPortal proxy.
    function deployOptimismPortal(
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        _output.optimismPortalProxy = IOptimismPortal(
            payable(deployProxy(_input.l2ChainId, _output.opChainProxyAdmin, _input.saltMixer, "OptimismPortal"))
        );
        return _output;
    }

    /// @notice Deploys the ETHLockbox proxy.
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        _output.ethLockboxProxy =
            IETHLockbox(deployProxy(_input.l2ChainId, _output.opChainProxyAdmin, _input.saltMixer, "ETHLockbox"));
        return _output;
    }

    /// @notice Deploys the SystemConfig proxy.
    ...
    {
        _output.systemConfigProxy =
            ISystemConfig(deployProxy(_input.l2ChainId, _output.opChainProxyAdmin, _input.saltMixer, "SystemConfig"));
        return _output;
    }

    /// @notice Deploys the OptimismMintableERC20Factory proxy.
    function deployOptimismMintableERC20Factory(
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        _output.optimismMintableERC20FactoryProxy = IOptimismMintableERC20Factory(
            deployProxy(_input.l2ChainId, _output.opChainProxyAdmin, _input.saltMixer, "OptimismMintableERC20Factory")
        );
        return _output;
    }

    /// @notice Deploys the DisputeGameFactory proxy.
    function deployDisputeGameFactory(
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        _output.disputeGameFactoryProxy = IDisputeGameFactory(
            deployProxy(_input.l2ChainId, _output.opChainProxyAdmin, _input.saltMixer, "DisputeGameFactory")
        );
        return _output;
    }

    /// @notice Deploys the AnchorStateRegistry proxy.
    function deployAnchorStateRegistry(
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        _output.anchorStateRegistryProxy = IAnchorStateRegistry(
            deployProxy(_input.l2ChainId, _output.opChainProxyAdmin, _input.saltMixer, "AnchorStateRegistry")
        );
        return _output;
    }

    /// @notice Deploys the L1StandardBridge proxy (ChugSplash).
    ...
    /// @notice Deploys the L1CrossDomainMessenger proxy (ResolvedDelegate).
    ...
    /// @notice Deploys the DelayedWETH proxy for permissioned games.
    ...
    /// @notice Sets implementations and initializes all deployed proxies.
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        ...
        // Initialize L1ERC721Bridge
        data = encodeL1ERC721BridgeInitializer(_output);
        upgradeToAndCall(
            _output.opChainProxyAdmin, address(_output.l1ERC721BridgeProxy), implementation.l1ERC721BridgeImpl, data
        );

        // Initialize SystemConfig (must be first - others depend on it)
        data = encodeSystemConfigInitializer(_input, _output, _superchainConfig);
        upgradeToAndCall(
            _output.opChainProxyAdmin, address(_output.systemConfigProxy), implementation.systemConfigImpl, data
        );

        // Set custom gas token feature if requested
        if (_input.useCustomGasToken) {
            _output.systemConfigProxy.setFeature(Features.CUSTOM_GAS_TOKEN, true);
        }

        // Set ETH lockbox feature if interop enabled
        if (isDevFeatureEnabled(DevFeatures.OPTIMISM_PORTAL_INTEROP)) {
            _output.systemConfigProxy.setFeature(Features.ETH_LOCKBOX, true);
        }

        // Initialize OptimismPortal
        if (isDevFeatureEnabled(DevFeatures.OPTIMISM_PORTAL_INTEROP)) {
            data = encodeOptimismPortalInteropInitializer(_output);
            upgradeToAndCall(
            ...
            data = encodeOptimismPortalInitializer(_output);
            upgradeToAndCall(
                _output.opChainProxyAdmin, address(_output.optimismPortalProxy), implementation.optimismPortalImpl, data
            );
        }
        ...
        data = encodeETHLockboxInitializer(_output, portals);
        upgradeToAndCall(_output.opChainProxyAdmin, address(_output.ethLockboxProxy), implementation.ethLockboxImpl, data);

        // Initialize OptimismMintableERC20Factory
        data = encodeOptimismMintableERC20FactoryInitializer(_output);
        upgradeToAndCall(
            _output.opChainProxyAdmin,
        ...
        data = encodeL1CrossDomainMessengerInitializer(_output);
        upgradeToAndCall(
            _output.opChainProxyAdmin,
        ...
        data = encodeL1StandardBridgeInitializer(_output);
        upgradeToAndCall(
            _output.opChainProxyAdmin, address(_output.l1StandardBridgeProxy), implementation.l1StandardBridgeImpl, data
        );

        // Initialize DelayedWETH
        data = encodeDelayedWETHInitializer(_output);
        upgradeToAndCall(
            _output.opChainProxyAdmin,
        ...
        // Register permissioned game
        _registerPermissionedGame(_input, implementation, _output);
        transferOwnership(address(_output.disputeGameFactoryProxy), address(_input.roles.opChainProxyAdminOwner));

        // Initialize AnchorStateRegistry
        data = encodeAnchorStateRegistryInitializer(_input, _output);
        upgradeToAndCall(
            _output.opChainProxyAdmin,
        ...
        // Transfer ownership of ProxyAdmin to final owner
        transferOwnership(address(_output.opChainProxyAdmin), _input.roles.opChainProxyAdminOwner);

        emit Deployed(_input.l2ChainId, _deployer, abi.encode(_output));
        return _output;
    }
}
...
contract OPContractsManager is ISemver {
    ...
    // -------- Split Deployment Functions for RSK Compatibility --------
    // These wrapper functions allow deploying OP Stack contracts in separate transactions
    // to fit within RSK's 6.8M block gas limit.
    ...
    /// @notice Deploys the L1StandardBridge proxy.
    ...
    /// @notice Deploys the L1CrossDomainMessenger proxy.
