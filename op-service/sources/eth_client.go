...
func (s *EthClient) headerCall(ctx context.Context, method string, id rpcBlockID) (eth.BlockInfo, error) {
	...
	// Run Info with trustCache=true so it skips the inline hash check; we
	// run the (possibly pluggable) verifier explicitly below.
	info, err := header.Info(true, s.mustBePostMerge)
	if err != nil {
		return nil, err
	}
	if !s.trustRPC {
		if err := s.runHeaderVerify(ctx, header); err != nil {
			return nil, err
		}
	}
...
func (s *EthClient) blockCall(ctx context.Context, method string, id rpcBlockID) (eth.BlockInfo, types.Transactions, error) {
	...
	// Run Info with trustCache=true so it skips the inline Verify(); we run
	// the (possibly pluggable) verifier explicitly below.
	info, txs, err := block.Info(true, s.mustBePostMerge)
	if err != nil {
		return nil, nil, err
	}
	if !s.trustRPC {
